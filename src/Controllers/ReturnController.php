...
<body>
     ...
     * @return Response
     */
    public function searchOrder(Request $request, Response $response): Response
    {
        ...
        try {
            ...
            if (!$order) {
                // If repository supports other lookups, implement here
                return $response->json([
                    'success' => false,
    ...
    /**
     * Track a return by return number
     * @param Twig $twig
     * @param string $returnNumber
     * @return string
     */
    public function trackByNumber(Twig $twig, string $returnNumber): string
    {
        try {
            $returnRecord = $this->returnRepository->findByReturnNumber($returnNumber);
            if (!$returnRecord) {
                return $twig->render('ReturnsPortal::content.return-error', [
                    'error' => 'Return not found'
                ]);
            }
            return $this->track($twig, (int)$returnRecord->id);
        } catch (\Exception $e) {
            return $twig->render('ReturnsPortal::content.return-error', [
